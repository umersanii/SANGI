...
class WorkspaceMonitor:
    ...
    def __init__(self, config_path='config.json'):
        ...
        self.notification_detector = NotificationDetector(
            logger=self.logger,
            callback=self.on_notification_received
        )
    ...
    def check_clipboard_activity(self):
        ...
        try:
            ...
            self.logger.debug("Clipboard check timeout")
        except FileNotFoundError:
            # xclip not installed - disable clipboard monitoring
            pass
        except Exception as e:
    ...
    def publish_emotion(self, emotion):
        ...
        # Validate emotion value is in valid range
        emotion_value = EMOTIONS.get(emotion, 0)
        ...
        try:
            success = self.mqtt_client.publish('sangi/emotion/set', payload, 1)
            
            if not success:
                self.logger.warning("MQTT publish returned False")
    ...
    def on_notification_received(self, notif_type, title, message):
        """Callback when notification is detected"""
        # Publish notification to SANGI
        payload = json.dumps({
            'type': notif_type,
            'title': title[:30],  # Truncate to fit ESP32 display
            'message': message[:60],
            'timestamp': int(time.time())
        })
        ...
        try:
            ...
            self.logger.info(f"📤 Notification sent to SANGI: [{notif_type}] {title}")
        except Exception as e:
